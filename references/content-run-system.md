...
每个生成版本必须在平台 `qa.md` 的媒体状态表登记：

| 文件 | 状态 | 退回或通过原因 |
| --- | --- | --- |
| `...-v01.png` | `working` / `rejected` / `publish` |  |

- `working`：仍在生成或等待单页 QA；
- `rejected`：因画幅、事实、文字、瓶型、礼盒结构、边缘或套图重复而退回；
- `publish`：通过单页 QA 和套图 QA，可进入发布清单。

发布总览只能引用 `publish` 文件。`prompts.md` 的最终版本索引、`publish.md` 的配图清单和交付说明也只能指向 `publish`；历史候选可以保留在 `media/`，但必须在 `qa.md` 写明 `rejected` 原因，不能以“最新生成”代替发布状态。
