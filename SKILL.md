...
8. **锁定小红书机位与画幅**：生成前填写逐页机位分配表，记录 `view_id`、主体、参考图和可测量透视特征；相邻页面不得使用相同 `view_id`。每页首轮原生成文件必须是宽:高 = 3:4，并立即执行 `python scripts/validate_xhs_image.py <image.png>`；不通过就按原提示词重新生成，不允许通过裁切、拉伸或补边把错误比例伪装成合格成图。
9. **保存发布文案**：把标题、正文/描述、标签、评论、CTA 和披露写入各平台 `publish.md`。
10. **保存生产资产**：小红书写入 `prompts.md`，视频写入 `storyboard.md`，媒体写入 `media/`。
11. **执行 QA**：把事实、文字、视觉、AIGC、合规与修改记录写入 `qa.md`。
12. **运行验证**：
...
13. **交付缺口**：列出仍需补拍、文件核验、库存核验或平台实时确认的项目。
